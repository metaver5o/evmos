...
if [ ! -f /home/evmos/.evmosd/config/genesis.json ]; then \
  ...
  evmosd add-genesis-account \"$EVMOS_ADDRESS\" 1000000000000aevmos --keyring-backend=$KEYRING_BACKEND && \
  \
  # Create validator with delegation and set min-self-delegation equal to that amount \
  evmosd gentx \"$KEY_NAME\" 1000000000000aevmos --chain-id=\"$CHAIN_ID\" --commission-rate=0.1 --min-self-delegation=1000000000000aevmos --keyring-backend=$KEYRING_BACKEND && \
